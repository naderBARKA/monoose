...
exports.addContact = async (req, res) => {
  try {
    const { firstname,lastename,email,age,favoriteFoods } = req.body;
    const findContact = await Contact.findOne({ email: email });
    if (findContact) {
      return res.status(400).send({ msg: "email is used!try an other one..." });
    }
    // console.log(req.body);
    const newContact = new Contact({ firstname,lastename,email,age,favoriteFoods });
    await newContact.save();
    res
      .status(200)
      .send({ msg: "contact added with succ"});
  } catch (error) {
...
exports.calledMaryContact = async (req, res) => {
  try {
    const deleteMary = await Contact.deleteMary({ firstname: /mary/i });
    res.status(200).send({
